...
import { Com_Profile_Controller } from "../../controller/Profiles/company_profile";
import CompanyProfileService from "../../service/Profiles/CompanyProfileService";
...
com_profile_router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const user = await com_profile_Controller.Get_Com_ProfileBy_Id(
        req.params.id
      );
      res.send(user);
    } catch (error) {
...
com_profile_router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const userDelete = await com_profile_Controller.Delete_Com_Profile(
        CardID
      );
      if (!userDelete) {
        res.status(404).json({ message: "User Not Found" });
